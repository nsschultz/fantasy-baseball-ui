...
const objectCompare = <T>(a: T, b: T): number => {
  if (a === b) return 0;
  if (a) return -1;
  return 1;
};

export const defaultObjectComparator = <T extends BaseEntity>(obj1: T, obj2: T, key: keyof T): number => {
  if (!obj1 || !obj2) return objectCompare(obj1, obj2);
  return compare(obj1[key], obj2[key]);
};

export const playerDefaultComparator = (player1?: Player, player2?: Player): number => {
...
export const playerNameComparator = (player1?: Player, player2?: Player): number => {
  if (!player1 || !player2) return objectCompare(player1, player2);
...
export const playerPositionsComparator = (player1?: Player, player2?: Player): number => {
  if (!player1 || !player2) return objectCompare(player1, player2);
...
export const playerTeamComparator = (player1?: Player, player2?: Player): number => {
  if (!player1 || !player2) return objectCompare(player1, player2);
